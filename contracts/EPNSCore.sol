...
contract EPNSCoreV2 is Ownable, ReentrancyGuard, Initializable {
    ...
    uint256 ADD_CHANNEL_MIN_POOL_CONTRIBUTION = 50; // 50 DAI or above to create the channel
    uint256 ADD_CHANNEL_MAX_POOL_CONTRIBUTION = 500 * 50 * 10**18; // 500 DAI or below, we don't want channel to make a costly mistake as well, for test it's 25k

    uint256 UPDATE_CHANNEL_POOL_CONTRIBUTION = 500 * 10**18; // 500 DAI or above to update the channel
    uint256 DELEGATED_CONTRACT_FEES = 1 * 10**17; // 0.1 DAI to perform any delegate call
    ...
    bytes EPNS_FIRST_MESSAGE_HASH =
        "3+QmZbff755tAPZ22REdF5PDLLjtbRUKu2THaXSZ7pkRf2qV";
    ...
    *************** */
    ...
    uint256 public groupHistoricalZ; // Abbre
    uint256 public groupLastUpdate; // The last update block number, used to calculate fair share
    uint256 public groupFairShareCount; // They are alias to channels count but seperating them for brevity

    /* For maintaining the #DeFi finances
     */
    uint256 public poolFunds; // Always in DAI
    ...
    function initialize() external {
        ...
        emit AddChannel(
            ...
            "QmTCKYL2HRbwD6nGNvFLe4wPvDNuaYGr6RiVeCvWjVpn5s"
        );
        emit AddChannel(
            msg.sender,
        ...
        _createChannel(
            0x0000000000000000000000000000000000000000,
            ChannelType.NonInterestBearing,
            0
        );
    ...
    modifier onlyChannelizationWhitelist(address _addr) {
        require(
            (msg.sender == owner() || channelizationWhitelist[_addr] == true),
    ...
    function removeFromChannelizationWhitelist(address _addr)
        external
        onlyOwner
    {
    ...
    function createChannelWithFees(bytes calldata _identity)
        external
        ...
        returns (uint256 ratio)
    {
        ...
        for (uint256 i = 0; i < users[msg.sender].subscribedCount; i++) {
            ...
            if (
                ...
                ChannelType.InterestBearingChannel
            ) {
    ...
    function calcSingleChannelEarnRatio(
        ...
        if (
            channels[_channel].channelType == ChannelType.InterestBearingChannel
        ) {
    ...
    function _createChannelWithFees() private {
        ...
        uint256 allowedAllowance = IERC20(daiAddress).allowance(
            msg.sender,
            address(this)
        );
